...
const emailTemplates = {
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px;">
        ...
        <div style="background-color: #f5f5f5; padding: 15px; border-radius: 5px; margin: 20px 0;">
          ...
          <p><strong>Department Charge to:</strong> ${prfData.projectCode || "Not specified"}</p>
    ...
    html: `
      <div style="font-family: 'Segoe UI', Arial, sans-serif; max-width: 650px; margin: 0 auto; padding: 0; background: linear-gradient(135deg, #f5f7fa 0%, #c3cfe2 100%);">
        ...
        <div style="background: linear-gradient(135deg, #E74C3C 0%, #C0392B 100%); padding: 30px 20px; text-align: center; border-radius: 8px 8px 0 0;">
          ...
          <h2 style="color: #2c3e50; margin: 0; font-size: 24px; font-weight: 600;">PRF Has Been Rejected</h2>
  ...
  stockAvailabilityCheck: (prfData) => {
    ...
    // ✅ Generate ONE approve/reject link for the whole PRF
    const approveLink = `${baseUrl}/stock/approve/form?prfId=${encodeURIComponent(
      prfData.prfId
    )}&prfNo=${encodeURIComponent(prfData.prfNo)}&checkerName=${encodeURIComponent(
      prfData.checkerName || ""
    )}`;
    ...
    const rejectLink = `${baseUrl}/stock/reject/form?prfId=${encodeURIComponent(
      ...
      prfData.checkerName || ""
    )}&reason=Stock%20not%20available`;

    // ✅ Generate STOCK TABLE dynamically
    const stockRows = prfData.stockItems
      .map(
        (item) => `
        <tr>
          <td style="padding: 10px; border: 1px solid #ddd;">${item.stockCode}</td>
          <td style="padding: 10px; border: 1px solid #ddd;">${item.stockName}</td>
          <td style="padding: 10px; border: 1px solid #ddd;">${item.quantity}</td>
          <td style="padding: 10px; border: 1px solid #ddd;">${item.uom}</td>
          <td style="padding: 10px; border: 1px solid #ddd;">${item.purpose || ""}</td>
        </tr>
      `
      )
      .join("");
    ...
    return {
      subject: `[URGENT] Stock & Stock Code Check Required - PRF #${prfData.prfNo}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px; background-color: #f9f9f9;">
          ...
          <div style="background: white; padding: 30px 20px; border-radius: 0 0 5px 5px;">
            <p style="color: #333; font-size: 16px;">Good Day,</p>
            
            <p style="color: #555; font-size: 15px; line-height: 1.6;">
              A new Purchase Request Form contains <strong>multiple IM stock codes</strong>.
            ...
            <div style="background: #FFF3E0; border-left: 5px solid #FF6B35; padding: 20px; margin: 25px 0; border-radius: 4px;">
              <p style="margin: 0 0 15px 0; font-weight: bold; color: #F7931E;">Purchase Request Details:</p>

              <table style="width: 100%; border-collapse: collapse;">
                <tr>
                  <td style="padding: 8px 0; font-weight: 600;">PRF Number:</td>
                  <td style="padding: 8px 0; font-weight: bold;">${prfData.prfNo}</td>
                </tr>
                <tr>
                  <td style="padding: 8px 0; font-weight: 600;">Prepared By:</td>
                  <td style="padding: 8px 0;">${prfData.preparedBy}</td>
                </tr>
                <tr>
                  <td style="padding: 8px 0; font-weight: 600;">Company:</td>
                  <td style="padding: 8px 0;">${prfData.company}</td>
                </tr>
              </table>
            </div>
            ...
            <div style="margin: 25px 0;">
              <p style="font-weight: bold; margin-bottom: 10px;">Stock Items:</p>

              <table style="width:100%; border-collapse: collapse;">
                <thead>
                  <tr style="background:#f2f2f2;">
                    <th style="padding:10px; border:1px solid #ddd;">Stock Code</th>
            ...
            <div style="text-align:center; margin: 30px 0;">
              <a href="${approveLink}" style="background:#E74C3C; color:#fff;padding:14px 30px;text-decoration:none;border-radius:4px;font-weight:bold;">
                ✓ NO STOCK AVAILABLE
              </a>
              &nbsp;&nbsp;
              <a href="${rejectLink}" style="background:#FFA500; color:#fff;padding:14px 30px;text-decoration:none;border-radius:4px;font-weight:bold;">
                📦 STOCK AVAILABLE (Use RIS)
              </a>
            </div>
  ...
  im07Notification: (prfData) => ({
    subject: `[PRF System] IM-07 Stock Code Requested - PRF #${prfData.prfNo}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 650px; margin:auto; border:1px solid #ddd; padding:20px;">
        <h2 style="color:#2c3e50;">IM-07 Stock Code Request Notification</h2>

        <p>Good Day,</p>

        <p>
          A Purchase Request has been created containing an <strong>IM-07 stock code</strong>.
        ...
        <div style="background:#f5f5f5; padding:15px; border-radius:5px;">
          <p><strong>Prepared By:</strong> ${prfData.preparedBy}</p>
          <p><strong>PRF Number:</strong> ${prfData.prfNo}</p>
          <p><strong>Stock Code:</strong> ${prfData.stockCode}</p>
          <p><strong>Stock Name:</strong> ${prfData.stockName}</p>
          <p><strong>Department Charge and Project Code:</strong> ${prfData.projectCode}</p>
        </div>
...
// Stock Code Matched Notification to Requestor
const getStockAvailableToRequestorTemplate = (prfData, approverName, reason) => {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px; background-color: #f9f9f9;">
      ...
      <div style="background: linear-gradient(135deg, #27AE60 0%, #229954 100%); padding: 25px 20px; text-align: center; border-radius: 5px 5px 0 0; color: white;">
        ...
        <h2 style="color:#2c3e50;" style="margin: 0; font-size: 24px; font-weight: bold;">STOCK AVAILABLE</h2>
      </div>
      ...
      <div style="background: white; padding: 30px 20px;">
        ...
        <p style="color: #27AE60; font-size: 16px; line-height: 1.6; font-weight: bold;">
          ✓ The requested stock is available please proceed to RIS.
        </p>
        ...
        <div style="background: #E8F8F5; border-left: 5px solid #27AE60; padding: 20px; margin: 25px 0; border-radius: 4px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
            ...
            <tr style="border-bottom: 1px solid #D5F4E6;">
              <td style="padding: 10px 0; color: #666; font-weight: 600;">Remarks:</td>
        ...
        <p style="color: #555; font-size: 14px; line-height: 1.6; margin: 20px 0;">
          Your requested item is currently available in stock. Please process this request through the RIS form instead of the PRF process.
        </p>
  ...
  return `
    <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px; background-color: #f9f9f9;">
      ...
      <div style="background: linear-gradient(135deg, #E74C3C 0%, #C0392B 100%); padding: 25px 20px; text-align: center; border-radius: 5px 5px 0 0; color: white;">
        <h2 style="margin: 0; font-size: 24px; font-weight: bold;">NO STOCK AVAILABLE & STOCK CODE MATCHED</h2>
      ...
      <div style="background: white; padding: 30px 20px;">
        ...
        <p style="color: #E67E22; font-size: 16px; line-height: 1.6; font-weight: bold;">
          ⚠️ The requested stock is currently not available you will now proceed to PRF.
        </p>
        ...
        <div style="background: #FADBD8; border-left: 5px solid #E74C3C; padding: 20px; margin: 25px 0; border-radius: 4px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin-top: 10px;">
            ...
            <tr style="border-bottom: 1px solid #F5B7B1;">
              ...
              <td style="padding: 10px 0; color: #333;">${rejectionReason}</td>
            </tr>
            <tr style="border-bottom: 1px solid #F5B7B1;">
        ...
        <p style="color: #555; font-size: 14px; line-height: 1.6; margin: 20px 0;">
          Since the requested item is not available in stock, the PRF approval process will continue for purchasing.
        </p>
...
// Wrong Stock Code Notification to Requestor
const getWrongStockCodeTemplate = (
  prfData,
  checkerName,
  rejectionReason
) => {
  return `
    <div style="font-family: Arial, sans-serif; max-width: 700px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 5px; background-color: #f9f9f9;">
      ...
      <div style="background: white; padding: 30px 20px;">
        ...
        <!-- Details -->
        <div style="background: #FDEDEC; border-left: 5px solid #E74C3C; padding: 20px; margin: 25px 0; border-radius: 4px;">

          <p style="margin: 0 0 15px 0; font-weight: bold; color: #C0392B; font-size: 16px;">
            Stock Code Validation Details:
          </p>

          <table style="width: 100%; border-collapse: collapse;">

            <tr style="border-bottom: 1px solid #F5B7B1;">
              <td style="padding: 10px 0; font-weight: 600; color: #666; width: 40%;">
                PRF Number:
              </td>

              <td style="padding: 10px 0; font-weight: bold; color: #C0392B;">
                ${prfData.prfNo}
              </td>
            </tr>

            <tr style="border-bottom: 1px solid #F5B7B1;">
              <td style="padding: 10px 0; font-weight: 600; color: #666;">
                Stock Code:
              </td>
            ...
            <tr>
              ...
              <td style="padding: 10px 0; color: #333;">
                ${rejectionReason || "Invalid or incorrect stock code"}
              </td>
            </tr>
        ...
        <p style="color: #555; font-size: 14px; line-height: 1.6;">
          Please review and correct the stock code before resubmitting the Purchase Request Form.
        </p>
...
const sendStockAvailabilityNotification = async (
  ...
  try {
    console.log("[v0] Starting stock availability notification for PRF:", prfNo);
    ...
    if (!Array.isArray(stockCheckRecipients)) {
      stockCheckRecipients = [stockCheckRecipients];
    }
...
const sendStockAvailableToRequestor = async ({
  ...
  const mailOptions = {
    ...
    to: toEmail,
    subject: `⚠️ No Stock Available - PRF Process Will Continue - PRF ${prfNo}`,
...
const sendStockNotAvailableToRequestor = async ({
  ...
  const mailOptions = {
    from: process.env.EMAIL_FROM || process.env.SMTP_USER,
    to: toEmail,
    subject: `📦 Stock Available - Use RIS - PRF ${prfNo}`,
...
const sendWrongStockCodeNotification = async ({
  toEmail,
  preparedBy,
  stockCode,
  stockName,
  ...
  const mailOptions = {
    ...
    subject: `❌ Wrong Stock Code - PRF ${prfNo}`,

    html: getWrongStockCodeTemplate(
      { prfNo, stockCode, stockName },
      verifiedBy,
      reason
    ),
...
// Notify Corplan Department if IM-07 stock code is requested
const sendIM07CorplanNotification = async (prfData) => {
  try {

    const corplanEmails = [
      "Harold.Dinglasan@nutratech.com.ph",
      "Harold.Dinglasan@nutratech.com.ph"
    ];

    const transporter = getTransporter();

    const template = emailTemplates.im07Notification(prfData);

    const results = [];

    for (const email of corplanEmails) {
      const mailOptions = {
        from: process.env.EMAIL_FROM || process.env.SMTP_USER,
        to: email,
      ...
      console.log(`✅ IM-07 notification sent to ${email}`);
    ...
    console.error("❌ IM-07 Corplan notification error:", error.message);
    return { success: false, error: error.message };
  }
};
...
module.exports = {
  ...
  sendIM07CorplanNotification,
  sendWrongStockCodeNotification
}
